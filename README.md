...
1.Initial Marking Mark every state pair (Si, Sj) as Distinguished (✗)
if they produce different outputs for any input.
...
3.Iterative Propagation Re-scan repeatedly — if any implied pair gets marked ✗,
mark the parent pair ✗ too. Repeat until no new markings.
4.Merge Equivalent States All unmarked pairs are Equivalent (✓).
Group them using Union-Find → these states can be merged.
